...
trait OneServerPerSuite extends AsyncTestSuiteMixin with ProcessProvider { this: AsyncTestSuite =>
  ...
  /** Runs this suite of tests.
    *
    * @param testName
    *   an optional name of one test to execute. If <code>None</code>, all relevant tests should be executed, i.e., <code>None</code> acts
    *   like a wildcard that means execute all relevant tests in this <code>Suite</code>.
    * @param args
